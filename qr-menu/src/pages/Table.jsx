...
function Table() {
 ...
 //TOAST- FUNCTION 
  const showSuccess = () => {
    toast.current.show({severity:'success', summary: 'Success', detail:'Generation was successful!', life: 3000});
  }
  const showInfo = () => {
      toast.current.show({severity:'info', summary: 'Info', detail:'Message Content', life: 3000});
  }
  const showWarn = () => {
      toast.current.show({severity:'warn', summary: 'Warning', detail:'warn - Warn Message!!', life: 3000});
  }
  const showError = () => {
...
//BTN SAVE
  const handleSave = () => {
    setSubmitted(true);
      const qrTotal = Array.from({ length: quantityState }, (_, index) => index * urlState);
      setTables((prevState) => ({
        ...prevState,
        id: qrCodeId.id + 1,
...
const footerContent = (
  <div>
      <Button label="No" icon="pi pi-times" onClick={hideDialogNo} className="p-button-text" />
      <Button label="Yes" icon="pi pi-check" onClick={hideDialogYes} autoFocus />
  </div>
);
        ...
        <div className='page-left'>
          ...
          <div className='page-right'>
            <div className='block-title'>
              <h1 className='page-title'>Tables</h1>
                    <div className='block-btn'>
                      <button onClick={addTable} className='btn-addtable'>
            ...
            <form onSubmit={generQr}>               
                  <Dialog visible={tableDialog} style={{ width: '32rem' }} breakpoints={{ '960px': '75vw', '641px': '90vw' }} header="Add Tables" modal className="p-fluid" footer={tableDialogFooter} onHide={hideDialogNo}>
                    <div className="field">
                    ...
                    <div>
                      ...
                      className={classNames({ 'p-invalid': submitted && !tableState.urlState })}
                      />
                      {submitted && !tableState.urlState && <small className="p-error">Name is required.</small>}
                    </div>
                    <div className='btn-addtable'>
                      <Button onClick={clickGenerateQrCode}         className='btn-addtable' style={{width: '50%'}}>
                          <svg xmlns="http://www.w3.org/2000/svg" width="16"    height="16" viewBox="0 0 16 16" fill="none">
                            <path d="M8.66665 1.33331H7.33331V7.33331H1.33331V8.66665H7.33331V14.6666H8.66665V8.66665H14.6666V7.33331H8.66665V1.33331Z" fill="white"/>
                          </svg>
                  ...
                  <div className="card flex justify-content-center" >
                    <Dialog 
                      ...
                      <p className="m-0">Are you sure that you want to generate ” `${quantityState }` ” QR codes  för “ `${quantityState }` ” tabkles?</p>
                    </Dialog>
                  </div>
            </div>
          </div>
